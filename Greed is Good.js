...
function score(dice) {
  ...
  for (let i = 2; i < 7; i++) 
    if (dice.filter(x => x == i).length > 2)
      s += i * 100;
  dice.filter(x => x == 5).length > 2 ?
    s += (dice.filter(x => x == 5).length - 3) * 50 :
    s += dice.filter(x => x == 5).length * 50;
  dice.filter(x => x == 1).length > 2 ?
    s += 1000 + (dice.filter(x => x == 1).length - 3) * 100 :
    s += dice.filter(x => x == 1).length * 100;
  return s;
}

function scoreBP(dice) {
  var dc = [0,0,0,0,0,0];
