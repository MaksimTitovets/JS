...
function formatDuration (seconds) {
  ...
  hrs -= day * 24;
  day -= yrs * 365;
  seconds == 0 ? 0 : seconds > 1 ? seconds += ' seconds' : seconds += ' second';
  min == 0 ? 0 : min > 1 ? min += ' minutes' : min += ' minute';
  hrs == 0 ? 0 : hrs > 1 ? hrs += ' hours' : hrs += ' hour';
  day == 0 ? 0 : day > 1 ? day += ' days' : day += ' day';
  yrs == 0 ? 0 : yrs > 1 ? yrs += ' years' : yrs += ' year';
  let a = [yrs, day, hrs, min, seconds].filter(x => x != 0);
  return a.map((x, i) => a[i + 2] ? x + ', ' : a[i + 1] ? x + ' and ' : x)
          .join('');
}

function formatDurationBP (seconds) {
