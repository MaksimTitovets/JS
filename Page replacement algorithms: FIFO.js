...
The idea is obvious from the name: the operating system keeps track of all the pages in memory in a queue, with the most recent arrival at the back,
...
The fifo function will take two parameters as input: the number of maximum pages that can be kept in memory at the same time n and a reference list containing numbers.
...
function fifo(n, referenceList) { 
  let arr = referenceList.filter((x, i) => referenceList.indexOf(x) == i)
                         .slice(0, n),
      p = 0;
  if (arr.length < n) 
    arr = arr.concat(Array(n - arr.length).fill(-1));
  for (let i = n; i < referenceList.length; i++) {
    if (arr.indexOf(referenceList[i]) < 0) {
      arr.splice(p, 1, referenceList[i]);
      p++;
      if (p > n - 1) 
        p = 0;
      }
